...
# Send a test query to the deployed policy_agent via A2A JSON-RPC.
#
# Usage:
#   ./agents/enterprise/policy_agent/test.sh                           # default question
#   ./agents/enterprise/policy_agent/test.sh -q "What is PTO policy?"  # custom question
...
SERVICE_NAME="${POLICY_AGENT_SERVICE_NAME:-policy-agent}"
...
CARD=$(curl -s -H "Authorization: Bearer $TOKEN" \
    "${SERVICE_URL}/a2a/policy_agent/.well-known/agent-card.json")

...
# Send a test query to the deployed benefits_agent via A2A JSON-RPC.
#
# Usage:
#   ./agents/enterprise/benefits_agent/test.sh                           # default question
#   ./agents/enterprise/benefits_agent/test.sh -q "What is PTO policy?"  # custom question
...
SERVICE_NAME="${POLICY_AGENT_SERVICE_NAME:-benefits-agent}"
...
CARD=$(curl -s -H "Authorization: Bearer $TOKEN" \
    "${SERVICE_URL}/a2a/benefits_agent/.well-known/agent-card.json")
...
RESPONSE=$(curl -s -X POST \
    ...
    "${SERVICE_URL}/a2a/benefits_agent" \
    -d "{
        \"jsonrpc\": \"2.0\",
        \"id\": \"${MSG_ID}\",
        \"method\": \"message/send\",

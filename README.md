...
| 3 Run the evolution job | ~14 min | ~26 min | ~3 h | After analyzing the failures, the system drafts several new versions of the skill and holds a tryout to pick the best. How many drafts it writes is the profile's setting: lite says 2, standard says 3, full lets the system decide (up to 5 per agent, for up to 3 agents, over several rounds). Every draft costs one tryout — put it live, run all questions again, grade — about 5-8 min each. More drafts, better odds of a strong winner, more minutes. |

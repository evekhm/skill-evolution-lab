...
| 3 Run the evolution job | ~14 min | ~26 min | ~3 h | This step writes several competing new skills (candidates) and tests each one by re-running all the questions against it. One test pass takes 5-8 minutes, so the time is set by how many candidates get tested. Lite: 2 candidates, for the supervisor only = 2 test passes. Standard: 3 candidates = 3 test passes. Full: the system decides for itself — up to 5 candidates, for up to all 3 agents, over several rounds = dozens of test passes, which is why it takes hours. |
| Held-out measurement | ~5 min (13 unseen) | ~8 min (25 unseen) | ~30 min (23 unseen) | Mechanics identical everywhere: fresh traffic on questions evolution never saw, winner scored, V0 restored and scored on the same set — the difference between those two grounded rates is the run's reported result. Profiles differ only in test-set size, which sets how much the number can be trusted: at lite one question moves it 7.7 points (directional), at standard 4 points, and full draws its 23 from the stratified split (the one to quote in a writeup). |

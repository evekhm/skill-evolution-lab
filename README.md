...
| Held-out measurement | ~5 min (13 unseen) | ~8 min (25 unseen) | ~30 min (23 unseen) | Mechanics identical everywhere: fresh traffic on questions evolution never saw, winner scored, V0 restored and scored on the same set — the difference between those two grounded rates is the run's reported result. Profiles differ only in test-set size, which sets how much the number can be trusted: 7.7pp resolution at lite (directional), 4pp at standard, and full draws its 23 from the stratified split (the one to quote in a writeup). |

...
| 3 Run the evolution job | ~14 min | ~26 min | ~3 h | Writes competing new skills (candidates) and tests each one. Testing one candidate = put it live, run every question through the agents again, grade the answers — about 5-8 min. Lite tests 2 candidates (supervisor only), standard 3; full lets the system decide (up to 5 candidates x 3 agents x several rounds = dozens of candidate tests, hence hours). |
